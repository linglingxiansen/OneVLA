...
_ACTION_TOKEN_MAX = 153712 # here only for fast_tokenizer, see OneVLA/model/modules/vlm/tools/add_qwen_special_tokens/README.md
...
class _QWen_VL_Interface(nn.Module):
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs):
        ...
        checkpoint_path = qwenvl_config.get("checkpoint_path", None)
        use_flash_attention = qwenvl_config.get("use_flash_attention", False)


        env_ckpt = os.getenv("ONEVLA_QWEN_CHECKPOINT")
        if env_ckpt:
            checkpoint_path = env_ckpt


        # Debug: Print checkpoint_path information
        print(f"[DEBUG] checkpoint_path from config: {checkpoint_path}")
        print(f"[DEBUG] checkpoint_path type: {type(checkpoint_path)}")

        # exit()
        if checkpoint_path:
            print(f"[DEBUG] checkpoint_path exists check: {os.path.exists(checkpoint_path)}")
            if not os.path.exists(checkpoint_path):
                print(f"[DEBUG] Checkpoint path does not exist: {checkpoint_path}")
                print(f"[DEBUG] Current working directory: {os.getcwd()}")
                print(f"[DEBUG] Absolute path: {os.path.abspath(checkpoint_path) if checkpoint_path else None}")

        # Load from PyTorch checkpoint if provided, otherwise load from HuggingFace
        if checkpoint_path and os.path.exists(checkpoint_path):
            print(f"Loading model from PyTorch checkpoint: {checkpoint_path}")
            from transformers import AutoConfig
            
            # Always load model config from base_vlm (not from checkpoint directory)
            # because checkpoint directory contains training config, not model config
            print(f"Loading model config from base_vlm: {model_id}")
            try:
                model_config = AutoConfig.from_pretrained(
                    model_id,
                    cache_dir='/e2e-data3/embodied-research-data/lingfeng/pretrained_model',
                    local_files_only=True,
                )
            except Exception as e:
                # If local loading fails, try without local_files_only (but this may require network)
                print(f"Warning: Failed to load config locally: {e}")
                print(f"Attempting to load config from model_id (may require network connection)...")
                model_config = AutoConfig.from_pretrained(
                    model_id,
                    cache_dir='/root/.cache/huggingface/hub',
                )
            
            # Create model without flash_attention
            ...
            model = model.to("cuda")
            print("Model loaded from PyTorch checkpoint successfully!")
        else:
        ...
        #                                           cache_dir='/home/mi/.cache/huggingface/hub')
        processor = AutoProcessor.from_pretrained('/e2e-data3/embodied-research-data/lingfeng/pretrained_model/models--Qwen--Qwen2.5-VL-3B-Instruct/snapshots/66285546d2b821cf421d4f5eb2576359d3770cd3')
    ...
    def build_qwenvl_inputs(self, images, instructions, solutions=None, **kwargs):
        ...
        if solutions is not None:
            ...
            action_token_max = _ACTION_TOKEN_MAX # here only for fast_tokenizer, see OneVLA/model/modules/vlm/tools/add_qwen_special_tokens/README.md
            ...
            for i in range(labels.size(0)):
                ...
                if nonzero_indices.numel() > 0:
                    ...
                    seq[:] = IGNORE_INDEX
                    RuntimeWarning (f"action token are on in yout tokenizer, plz see OneVLA/model/modules/vlm/tools/add_qwen_special_tokens/README.md.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_yaml", type=str, default="./OneVLA/config/training/onevla_cotrain_oxe.yaml", help="Path to YAML config")
